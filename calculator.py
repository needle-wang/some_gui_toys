...
if sys.version_info.major == 3:
  from tkinter import Tk, W, E, RIGHT, StringVar, DISABLED
...
class Example(Frame):
    ...
    def __init__(self, parent):
        ...
        # 在文本区域显示的表达式字符串
        self.eval_buff = StringVar(value=0)
    ...
    def initUI(self):
        ...
        if sys.version_info.major == 2:
          showwarning(title = '注意', message = '使用的python版本是python2')
        ...
        lbl = Button(self, text="占位符", command=self.focus_set, state=DISABLED)
    ...
    def setNum(self, button_num):
        ...
        if button_num == '.' and '.' in self.eval_buff.get() :
            return
        if self.eval_buff.get() == '0':
            if button_num.isdigit():
                self.eval_buff.set(button_num)
        else:
            if self.eval_buff.get()[-1:] in ('+', '-', '*', '/') and\
               button_num in ('+', '-', '*', '/'):
                self.eval_buff.set(self.eval_buff.get()[:-1] + button_num)
            else:
    ...
    def remove_last_one_char(self):
        ...
        if len(self.eval_buff.get()[0:-1]) == 0:
            self.eval_buff.set(0)
        else:
